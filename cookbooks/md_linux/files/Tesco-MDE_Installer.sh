...
#	Note: The script has been modified to conform to Tesco standards to install MDE silently. 
#	      It will register with the org based on the package subscription.
#	      Command line to Install : sh Tesco-MDE_Installer.sh --install
#
#============================================================================
...
print_state()
{
	...
	# Additional configuration states - SK
	log_info "[S] Health Status: $(mdatp health --field healthy)"
	log_info "[S] Product Expiration: $(mdatp health --field product_expiration)"
    fi
}
...
install_on_fedora()
{
    ...
    log_info "[>] Set Configuration Profile"
    run_quietly "mkdir -p /etc/opt/microsoft/mdatp/managed && cp mdatp_managed.json /etc/opt/microsoft/mdatp/managed" "unable to set configuration profile ($?)" $ERR_MDE_NOT_CONFIGURED

    ### Add cron job entry to check and update MDATP version ###
    log_info "[>] Set Crontab job to update MDATP monthly"

    crontab -l > cron_backup
    echo "CRON_TZ=Europe/London" >> cron_backup
    echo "0 2 1 * * sudo yum update mdatp -y >> ~/mdatp_cron_job.log" >> cron_backup
    crontab cron_backup
    rm cron_backup
	
	sleep 5
	log_info "[v] configured"
}
